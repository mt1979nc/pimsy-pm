...
| **Learning Center PIMSY how-tos (v1.16.5)** | **Shipped** | Customer LC is Dock LC content for **using PIMSY** (Getting started, Password & access, Scheduling, Notes, Providers, Training 1–5). Wires Storylane + public Getting Started/Tips PDFs; omits signed Customer Guide. No invented dock.us URLs. No migrate; `npm run db:seed -- --templates-only`. |
...
Sequence: **v1.7 About + waiting-on (shipped)** → **v1.8 Prism Management (shipped)** → **v1.9 playbook/staffing (shipped)** → **v1.10 Forecast (shipped)** → **v1.11 Prism cutover (shipped)** → **Milestone: dark standalone Prism** (Alexander confirms). **v1.14 Waves A–C (shipped on live)** through customer digests (#39). Footer **v1.16.5**.
