      ...
      <div className="grid gap-5 [&>*]:min-w-0 lg:grid-cols-[1.3fr_1fr]">
        ...
        <div className="space-y-5">
          <Card>
            <CardHeader
              title="Waiting on"
              subtitle="Open shared threads — ball in court"
              action={
                <Link href="/reports/waiting-on" className="text-[12.5px] font-medium text-brand hover:underline">
                  WIP rollup →
                </Link>
              }
            />
            {waiting.length === 0 ? (
              <EmptyState title="Nothing outstanding" description="No open shared threads across the book." />
            ) : (
              <div className="px-4 py-3 text-[13px] text-ink-2">
                <div className="flex flex-wrap gap-3">
                  <span>
                    <span className="font-semibold text-ink">{waiting.length}</span> projects
                  </span>
                  <span>
                    <span className="font-semibold text-ink">{waitingPimsy}</span> on PIMSY
                  </span>
                  <span>
                    <span className="font-semibold text-ink">{waitingCustomer}</span> on customer
                  </span>
                </div>
              </div>
            )}
