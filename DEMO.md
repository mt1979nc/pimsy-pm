...
2. Sign in as staff (`alexander@pimsyehr.com` or another `@pimsyehr.com` account).
3. Open project **IMP-9001** (or the demo customer from seed). If screens look empty:
   - Local: `npm run setup` then `npm run demo:content`
...
5. Optional: Management → Alerts → show Teams webhook is available (don’t need it live).
...
2. Pilot FBH or OCE (or a safe WIP) in parallel with Dock.
3. Visual template editor + `.ics` for training dates.
4. Optional: Entra SSO for staff (Google SSO already optional).
